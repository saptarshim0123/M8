...
import { z } from 'zod'
import { LuCamera, LuFolder, LuSun, LuMoon, LuCopy, LuCheck, LuShieldCheck } from 'react-icons/lu'
...
const profileSchema = z.object({
    ...
    specialization: z.string().max(100, 'Too long').optional(),
})
    ...
    return (
        <div className="w-full max-w-2xl mx-auto px-6 py-8">
                ...
                <div className="flex flex-col items-center text-center mb-8">
                    <div className="relative group cursor-pointer" onClick={() => fileInputRef.current?.click()}>
                        {avatarPreview ? (
                            ...
                            <Avatar size={96} name={user?.name} variant="beam"
                    ...
                    <div className="flex gap-2 mt-3">
                        <button type="button" onClick={() => { fileInputRef.current.removeAttribute('capture'); fileInputRef.current.click() }}
                            className="btn btn-ghost btn-sm gap-1 font-data">
                            <LuFolder className="w-3 h-3" /> Gallery
                        </button>
                        <button type="button" onClick={() => { fileInputRef.current.setAttribute('capture', 'environment'); fileInputRef.current.click() }}
                            className="btn btn-ghost btn-sm gap-1 font-data">
                    ...
                    {user?.specialization && (
                        <p className="font-sans text-sm text-primary mt-1">{user?.specialization}</p>
                    )}
                    {user?.bio && (
                    ...
                    <div className="flex items-center gap-2 mt-3">
                        <span className="badge badge-success gap-1">
                            <LuShieldCheck size={12} /> Verified Therapist
                        </span>
                    </div>

                    {/* Practice Code */}
                    {user?.practiceCode && (
                        <div className="mt-3 flex items-center gap-2 bg-base-200 px-4 py-2 rounded-xl">
                            <span className="text-xs text-neutral/50 font-data">Practice Code:</span>
                            <span className="font-mono font-bold text-lg tracking-widest text-primary">{user.practiceCode}</span>
                            <button onClick={handleCopyCode} className="btn btn-ghost btn-xs btn-square">
                                {copied ? <LuCheck size={14} className="text-success" /> : <LuCopy size={14} />}
                            </button>
                        </div>
                    )}
                </div>
                ...
                <div className="tabs tabs-border mb-6">
                    <button className={`tab font-data ${activeTab === 'profile' ? 'tab-active' : ''}`}
                        onClick={() => setActiveTab('profile')}>Edit Profile</button>
                    <button className={`tab font-data ${activeTab === 'password' ? 'tab-active' : ''}`}
                        onClick={() => setActiveTab('password')}>Password</button>
                    <button className={`tab font-data ${activeTab === 'account' ? 'tab-active' : ''}`}
                ...
                {activeTab === 'profile' && (
                    <form onSubmit={profileForm.handleSubmit(onProfileSubmit)} className="flex flex-col gap-4">
                        <div>
                            ...
                            <input {...profileForm.register('name')} className="input input-bordered w-full" placeholder="Your name" />
                            {profileForm.formState.errors.name && (
                                <p className="text-error text-xs mt-1">{profileForm.formState.errors.name.message}</p>
                            )}
                        </div>
                        <div>
                            <label className="label font-data text-sm">Specialization</label>
                            <input {...profileForm.register('specialization')} className="input input-bordered w-full" placeholder="e.g. CBT, Anxiety, Depression" />
                            {profileForm.formState.errors.specialization && (
                                <p className="text-error text-xs mt-1">{profileForm.formState.errors.specialization.message}</p>
                            )}
                        </div>
                        <div>
                            <label className="label font-data text-sm">Bio</label>
                            <textarea {...profileForm.register('bio')} className="textarea textarea-bordered w-full"
                                placeholder="Tell your patients about yourself..." rows={3} />
                        ...
                        <div className="bg-base-200 rounded-xl p-4">
                            <p className="text-xs font-semibold text-neutral/50 mb-2">License Information (read-only)</p>
                            <div className="flex gap-4 text-sm">
                                <div>
                                    <span className="text-neutral/40">License #:</span>
                                    <span className="ml-1 font-mono">{user?.licenseNumber || '—'}</span>
                                </div>
                            </div>
                        </div>
                ...
                {activeTab === 'password' && (
                    <form onSubmit={passwordForm.handleSubmit(onPasswordSubmit)} className="flex flex-col gap-4">
                        <div>
                            ...
                            <input {...passwordForm.register('oldPassword')} type="password"
                                className="input input-bordered w-full" placeholder="Current password" />
                        ...
                        <div>
                            ...
                            <input {...passwordForm.register('newPassword')} type="password"
                                className="input input-bordered w-full" placeholder="New password" />
                        ...
                        <div>
                            ...
                            <input {...passwordForm.register('confirmPassword')} type="password"
                                className="input input-bordered w-full" placeholder="Confirm new password" />
                    ...
                    <div className="flex flex-col gap-4">
                        ...
                        <div className="card bg-error/5 border border-error/20 rounded-2xl p-6">
                            ...
                            <p className="font-sans text-sm text-neutral/60 mb-4">
                                Permanently deletes your account and all patient connections. Cannot be undone.
                            </p>
                ...
                <dialog id="deleteAccountModal" className="modal">
                    <div className="modal-box">
                        ...
                        <p className="font-sans text-sm text-neutral/70 mt-2">
                            This will permanently delete your account and all patient connections. This cannot be undone.
                        </p>
